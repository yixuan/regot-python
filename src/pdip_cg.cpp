...
void pdip_cg_internal(
    ...
    for (int iteration = 0; iteration < max_iter; ++iteration) {
        ...
        if (!std::isfinite(current_mar_err) || !std::isfinite(primal_gap) || !std::isfinite(dual_gap) || !std::isfinite(mu)) {
            // 数值已经失稳：直接退出，让外层把 converged=false 作为信号。
            break;
        }
        // 边际误差爆炸时早停，避免继续迭代产生 NaN plan、浪费墙钟时间
        ...
        // gap+μ 用 tol；mar / 稳定窗用 opts.cg_mar_tol（默认 1e-10）
