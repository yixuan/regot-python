...
// Unified entry for symmetric positive definite factorization:
// - Prefer in-place factorization (fewer large matrix copies);
// - On failure, retry with fixed diagonal shifts;
// - Returns a reference to the lower-triangular factor usable for solve.
static const std::vector<double>& factorize_with_fallback(
    ...
    // Fallback: one forced attempt at the largest shift for stable behavior.
    B_chol_work = B_backup;
...
void pdip_fp_internal(
    ...
    for (int iteration = 0; iteration < max_iter; ++iteration) {
        ...
        // 2.2 Build Newton system matrix (B1 vs B2 path by sparsity)
        ...
        // 2.3 Cholesky: sparse SimplicialLLT when B2 path succeeds; else dense with shift fallback
        t_loop = Clock::now();
        ...
        if (!use_fp_this_iter) {
            t_loop = Clock::now();
            pdip_cholesky_solve(N_block, *B_chol_ptr, c.data(), delta_lambda.data(), res_fp.data());
        ...
        // 2.5 Corrector step (Mehrotra-style)
        t_loop = Clock::now();
        ...
        if (!use_fp_this_iter) {
            t_loop = Clock::now();
            pdip_cholesky_solve(N_block, *B_chol_ptr, c.data(), delta_lambda_final.data(), res_fp.data());
            prof.cholesky_solve_sec += elapsed(t_loop);
            prof.cholesky_solve_calls += 1;
        } else {
            t_loop = Clock::now();
        ...
        // Record marginal error (always appended; whether it affects stopping is fp_stop_gap_mu_only)
        ...
        for (int i = 0; i < n_vars; ++i) mu += x[i] * s[i];
        mu /= n_vars;
        // Stop: default (rp and rd and mu); if fp_stop_gap_mu_only==false, also allow early exit via mar
