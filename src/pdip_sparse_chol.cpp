...
bool PdipSparseChol::factor(const BlockB2& blk)
{
    ...
    auto build_triplets = [&](double shift) {
        tri_buf_.clear();
        tri_buf_.reserve(static_cast<std::size_t>(M + n12 + 2 * blk.B12_val.size() + 64));
        for (int i = 0; i < M; ++i)
        {
            tri_buf_.emplace_back(i, i, blk.B11[static_cast<std::size_t>(i)] + shift);
        }
        for (int j = 0; j < n12; ++j)
        {
            tri_buf_.emplace_back(M + j, M + j, blk.B22[static_cast<std::size_t>(j)] + shift);
        }
        for (int i = 0; i < M; ++i)
        {
            for (int p = blk.B12_row_ptr[static_cast<std::size_t>(i)];
                 p < blk.B12_row_ptr[static_cast<std::size_t>(i) + 1]; ++p)
            {
                const int jj = blk.B12_col_idx[static_cast<std::size_t>(p)];
                const double v = blk.B12_val[static_cast<std::size_t>(p)];
